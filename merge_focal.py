""" Try to merge Focal Loss """

import tensorflow as tf

class Focal_MultiboxLoss(object):
    ...
    def _focal_loss(self, y_true, y_pred, weights=None):
        """Compute focal loss for predictions.
            Multi-labels Focal loss formula:
            FL = -alpha * (z-p)^gamma * log(p) -(1-alpha) * p^gamma * log(1-p)
                 ,which alpha = 0.25, gamma = 2, p = sigmoid(x), z = y_ture.
            Args:
                y_pred: A float tensor of shape [batch_size, num_anchors, num_classes] representing the predicted logits for each class
                y_ture: A float tensor of shape [batch_size, num_anchors, num_classes] representing one-hot encoded classification targets
        ...
        # y_ture > zeros <=> z=1, so poitive coefficient = z - p.
        pos_p_sub = array_ops.where(y_ture > zeros, y_ture - sigmoid_p, zeros)
    
        # For negative prediction, only need consider back part loss, front part is 0;
        # y_ture > zeros <=> z=1, so negative coefficient = 0.
        neg_p_sub = array_ops.where(y_ture > zeros, zeros, sigmoid_p)
